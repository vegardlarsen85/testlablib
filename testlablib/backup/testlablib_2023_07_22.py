...
ch.global_config.dtype = np.float64
...
class __Reactor_Common__(FeatureBooster):
    ...
    def __get_equilibrium_liq_K__(self, LiquidStreamIn):
        ...
        d = LiquidStreamIn.rxn_insta
        num_of_samples = LiquidStreamIn.temp_K.shape[0]
        num_of_reactions = LiquidStreamIn.num_of_rxn_insta
...
class LiquidStream(FeatureBooster):
    ...
    def set_solution_temp_K(self, value):
        self.temp_K = ch.Variable(data=value.astype(np.float64))

    def set_solution_flow_kg_h(self, value):
        self.flow_kg_h = ch.Variable(data=value.astype(np.float64))

    def set_specie_mass_fraction(self, id, value):
        self.specie[id]["Mass Fraction"] = ch.Variable(data=value.astype(np.float64))
    ...
    def set_species_molar_fraction(self, molar_fractions):
        ...
        for id in self.specie.keys():
            self.specie[id]["Mass Fraction"] = ch.Variable(data=self.specie[id]["Mass Fraction"].astype(np.float64))
    ...
    def __mass_fractions_dic2vec__(self):
        ...
        for id in self.specie.keys():
            i = self.specie[id]["Index"]
            wi = self.get_specie_mass_fraction(id=id)
            wi = F.expand_dims(wi, axis=1)
            if i == 0:
                w = wi
            else:
                w = F.concat((w, wi), axis=1)
        return w

    def __mass_fractions_vec2dic__(self, w):
        for i, id in enumerate(self.specie.keys()):
            self.specie[id]["Mass Fraction"] = F.get_item(w, slices=((slice(0, w.shape[0], 1)), i))
...
class Equilibrium_Liquid(__Reactor_Common__):

    def __init__(self, isothermal, adiabatic):
        super().__init__()
        self.isothermal = isothermal
        self.adiabatic = adiabatic

    def react(self, LiquidStreamIn, lr=0.5):
        ...
        self._b0_ = F.einsum("cw,sw->sc", self._matrix_["A"], self._w0_)
        ...
        for id in self.LiquidStreamOut.specie.keys():
            ...
            self.LiquidStreamOut.specie[id]["Mass Fraction"] = F.relu(self.LiquidStreamOut.specie[id]["Mass Fraction"])
        ...
        while converged == False:
            ...
            f = self.__isothermal_f__()
            dfdw = self.__isothermal_dfdw__()
    ...
    def __isothermal_f__(self):

        self.K = self.__get_equilibrium_liq_K__(self.LiquidStreamOut)
        reactants = 1.0 * self.K
        products = np.ones(shape=(self._num_of_samples_, self._num_of_rxn_insta_), dtype=np.float64)
        for j, jd in enumerate(self.LiquidStreamOut.rxn_insta.keys()):
            for id in self.LiquidStreamOut.rxn_insta[jd]["Stoch"].keys():
                nu = self.LiquidStreamOut.rxn_insta[jd]["Stoch"][id]
                if nu > 0:
                    products[:, j] = products[:, j] * self.LiquidStreamOut.get_specie_mass_fraction(id=id) ** np.abs(nu)
                else:
                    reactants[:, j] = reactants[:, j] * self.LiquidStreamOut.get_specie_mass_fraction(id=id) ** np.abs(nu)

        self.forward_rate_kmol_m3s = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_reversible_))
        self.backward_rate_kmol_m3s = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_reversible_))
        for i, id in enumerate(self.LiquidStreamOut.rxn_reversible.keys()):
            self.forward_rate_kmol_m3s[:, i], self.backward_rate_kmol_m3s[:, i] = \
            self.LiquidStreamOut.rxn_reversible[id]["Rate [kmol/m3.s]"](self.LiquidStreamOut)
        self.rate_kmol_m3s = self.forward_rate_kmol_m3s - self.backward_rate_kmol_m3s

        f1 = reactants - products
        f2 = self.rate_kmol_m3s
        f3 = np.einsum("cw,sw->sc", self._matrix_["A"], self._w_) - self._b0_
        #f = np.concatenate((f1, f2, f3), axis=1)
        return f1, f2, f3

    def __isothermal_dfdw__(self):

        drdw = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_reversible_, self._num_of_species_), dtype=np.float64)
        for rxn_i, rxn_id in enumerate(self.LiquidStreamOut.rxn_reversible.keys()):
            for specie_id in self.LiquidStreamOut.specie.keys():
                ...
                dw_probe = np.maximum(0.025 * w_probe, self.dw_min_td)
                self.LiquidStreamOut.set_specie_mass_fraction(id=specie_id, value=w_probe + dw_probe)
                forward_rate_pertubation, backward_rate_pertubation = self.LiquidStreamOut.rxn_reversible[rxn_id]["Rate [kmol/m3.s]"](self.LiquidStreamOut)
        ...
        dldw1 = self.__get_equilibrium_liq_dldw__(self.LiquidStreamOut, self.K)
        dldw2 = drdw
        dldw3 = self._matrix_["A broadcast"]
        dldw = np.concatenate((dldw1, dldw2, dldw3), axis=1)
        return dldw

    def __adiabatic_f__(self):
        pass

    def __adiabatic_dfdX__(self):
        pass
...
class Equilibrium_Liquid_Adiabatic(__Reactor_Common__):
    ...
    def react(self, LiquidStreamIn, lr=0.5):
        LiquidStreamOut = self.__react__(LiquidStreamIn, lr)
        return LiquidStreamOut

    def __react__(self, LiquidStreamIn, lr):

        R = self.__load_matrix_of_stochiometry_rxn__(LiquidStreamIn)
        self.matrix = self.__load_matrix_of_projection_and_conservation__(R)
...
class Equilibrium_Liquid_QPFlash(__Reactor_Common__):
...
class Equilibrium_Liquid_TPFlash(__Reactor_Common__):
...
class HeatExchanger_LiquidPlugFlow_CounterCurrent(FeatureBooster):
...
class HeatExchanger_LiquidPlugFlow_CounterCurrent_NTU(FeatureBooster):
...
class LiquidReactor_CSTR_Isothermal(__Reactor_Common__):
    ...
    def react(self, LiquidStreamIn, volume_m3):
        ...
        self._num_of_samples_ = LiquidStreamIn.temp_K.shape[0]
        self._num_of_reactions_ = LiquidStreamIn.num_of_rxn_reversible
        self._num_of_equilibriums_ = LiquidStreamIn.num_of_rxn_insta
        self._num_of_species_ = LiquidStreamIn.num_of_species
    ...
    def __f__(self):
        ...
        for i, id in enumerate(self.LiquidStreamOut.rxn_reversible.keys()):
            self.forward_rate_kmol_m3s[:, i], self.backward_rate_kmol_m3s[:, i] = self.LiquidStreamOut.rxn_reversible[id]["Rate [kmol/m3.s]"](self.LiquidStreamOut)
        self.rate_kmol_m3s = self.forward_rate_kmol_m3s - self.backward_rate_kmol_m3s

        self.K = self.__get_equilibrium_liq_K__(self.LiquidStreamOut)
        l1 = self.__get_equilibrium_liq_l__(self.LiquidStreamOut, self.K)
