...
class _GasLiquidContactor_PFR():
    ...
    def __system_dynamics_liq__(self, X, z):
        ...
        dmdz_liq = self._cross_sectional_area_m2 * (self.M_liq[None,:] * aJ_specie_liq + self._void_fraction_m3_m3 * 0.1 * self.M_liq[None,:] * r)
        ...
        for _ in range(1):
            ...
            q_lat = (m_gas_tot + m_liq_tot) * np.einsum("sr,sr->s", h, drdz) / self._cross_sectional_area_m2  # [kW/m3] = [kg/s] [kJ/kmol] [kmol/kg.m] * [1/m2]
    ...
    def __system_dynamics_gas__(self, X, z):
        ...
        dndz = self._cross_sectional_area_m2 * (aJ_specie_gas + self._void_fraction_m3_m3 * (1 - 0.1) * r)
    ...
    def __runge_kutta_liq__(self, f, X, z, order):

        if order == 1:

            dXdz = f(X, z)
            h = self.__step_size_liq__(X, dXdz, z)
            ...
            dXdz1 = f(X, z)
            h = self.__step_size_liq__(X, dXdz1, z)
            dX1 = h * dXdz1
            ...
            dXdz2 = f(X + 0.5 * dX1, z + 0.5 * h)
            dX2 = h * dXdz2

            dXdz3 = f(X + 0.5 * dX2, z + 0.5 * h)
            dX3 = h * dXdz3
    ...
    def __runge_kutta_gas__(self, f, X, z, order):

        if order == 1:
            dXdz = f(X, z)
            h = self.__step_size_gas__(X, dXdz, z)
            X_new = X + dXdz * h
            z_new = z + h

        elif order == 2:
            dXdz1 = f(X, z)
            h = self.__step_size_gas__(X, dXdz1, z)
            dX1 = h * dXdz1
            dXdz2 = f(X + dX1, z + h)
            X_new = X + 0.5 * (dXdz1 + dXdz2) * h
            z_new = z + h
    ...
    def __step_size_liq__(self, X, dXdz, z):
    ...
    def __step_size_gas__(self, X, dXdz, z):
...
class GasLiquidContactor_PFR_CounterCurrent(_GasLiquidContactor_PFR ,_Stochiometry, _Serializer, _LiquidEquilibrium, _StreamFunctions):
    ...
    def __react__(self, GasStreamIn, LiquidStreamIn, step_size_m, max_epochs=np.inf):
        ...
        while converged == False:
            ...
            while z > self.position_m[0]:
                ...
                X_new, z_new = self.__runge_kutta_liq__(self.__system_dynamics_liq__, X, z, order=1)
                m_new = X_new[:,0]
                T_new = X_new[:,1]
                w_new = X_new[:,2::]
            ...
            while z < self.position_m[-1]:
                ...
                X_new, z_new = self.__runge_kutta_gas__(self.__system_dynamics_gas__, X, z, order=1)
                n_new = X_new[:, 0]
                T_new = X_new[:, 1]
                y_new = X_new[:, 2::]
            ...
            gas_converged = np.min(gas_converged, axis=1, keepdims=False)
            sample_converged = np.minimum(liq_converged, gas_converged)
            sample_converged = sample_converged * (np.abs(dT_gas) < 0.2)
            sample_converged = sample_converged * (np.abs(dT_liq) < 0.2)
            converged = np.min(sample_converged, axis=0)
            w_old = 1.0 * w
            y_old = 1.0 * y
            T_old_liq = 1.0 * T_liq
            ...
            #converged = converged * (self.epochs > 20)
