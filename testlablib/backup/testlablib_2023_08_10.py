...
class __GasEquilibrium__:
    ...
    def __get_GasEquilibrium_exothermic_heat_kJ_kmol__(self, GasStreamIn, Kp, dKpdT):
        dT = 0.05
        T = LiquidStreamIn.temp_K
        T1 = T + dT
        Kp1 = Kp + dKpdT * dT
        q = 8.314 * (np.log(Kp1) - np.log(Kp)) / ((1 / T1) - (1 / T))
        return q
...
class LiquidCSTR_Isothermal(__Stochiometry__, __Serializer__, __LiquidEquilibrium__, __LiquidCSTR__):
...
class LiquidCSTR_Adiabatic(__Stochiometry__, __Serializer__, __LiquidEquilibrium__, __LiquidCSTR__):
...
class FlashEvaporation_QP_LiquidCSTR(__Stochiometry__, __Serializer__, __LiquidEquilibrium__, __LiquidCSTR__):
    ...
    def react(self, LiquidStreamIn, volume_m3, heat_kW, pressure_bara, lr=0.75):
        ...
        # Generate "GasStream"
        self.GasStreamOut = GasStream()
        l = Library()
        for id in self.LiquidStreamInit.vapor_pressure_bara.keys():
            gas_id = list(self.LiquidStreamInit.vapor_pressure_bara[id]["Stoch Gas"].keys())[0]
            liq_id = list(self.LiquidStreamInit.vapor_pressure_bara[id]["Stoch Gas"].keys())[0]
            molar_mass = self.LiquidStreamInit.get_specie_molar_mass_kg_kmol(id=liq_id)
            charge = self.LiquidStreamInit.get_specie_charge(id=liq_id)
            l.add_GasStream_specie_gas(id=gas_id, molar_mass_kg_kmol=molar_mass, charge=charge)
        for id in self.LiquidStreamInit.vapor_pressure_bara.keys():
            gas_id = list(self.LiquidStreamInit.vapor_pressure_bara[id]["Stoch Gas"].keys())[0]
            self.GasStreamOut.add_specie(id=gas_id, library=l)
        self.GasStreamOut.set_gas_flow_kmol_h(value=np.zeros(shape=(LiquidStreamIn.temp_K.shape[0],), dtype=np.float64))
        for id in self.GasStreamOut.specie.keys():
            self.GasStreamOut.set_specie_molar_fraction(id=id, value=np.ones(shape=(LiquidStreamIn.temp_K.shape[0],), dtype=np.float64) / self.GasStreamOut.num_of_species)
    ...
    def __get_FlashEvaporation_f_vapor_pressure__(self, LiquidStreamIn, GasStreamIn, pressure_bara):
        vapor_pressure_bara = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_vapor_pressure_bara), dtype=np.float64)
        gas_flow = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_vapor_pressure_bara), dtype=np.float64)
        for i, id in enumerate(LiquidStreamIn.vapor_pressure_bara.keys()):
            ...
            vapor_pressure_bara[:,i] = LiquidStreamIn.get_specie_vapor_pressure_bara(gas_id)
            gas_flow[:,i] = GasStreamIn.get_specie_flow_kmol_h(gas_id)
        f_vapor_pressure = vapor_pressure_bara / pressure_bara[:,None] - gas_flow / np.sum(gas_flow, axis=1, keepdims=True)
        return f_vapor_pressure

    def __get_FlashEvaporation_f_mass_balance__(self, LiquidStreamIn, GasStreamIn, matrix, b):
        m_tot_liq = LiquidStreamIn.get_solution_flow_kg_h()
        m_tot_gas = GasStreamIn.get_gas_flow_kg_h()
        m_tot = m_tot_liq + m_tot_liq
        m_liq = m_tot_liq[:, None] * LiquidStreamIn.__mass_fractions_dic2vec__()
        m_gas = m_tot_gas[:, None] * GasStreamIn.__molefrac2massfrac__(y=GasStreamIn.__molar_fractions_dic2vec__())
        w_liq = m_liq / m_tot[:, None]
        w_gas = m_gas / m_tot[:, None]
        f_mass_balance = np.einsum("cw,sw->sc", matrix["A"], LiquidStreamIn.__mass_fractions_dic2vec__()) - b
        return f_mass_balance

    def __get_FlashEvaporation_f_energy_balance__(self):
        pass

    def __get_FlashEvaporation_dfdm_vapor_pressure__(self, LiquidStreamIn):
        pass

    def __get_FlashEvaporation_dfdT_vapor_pressure__(self, LiquidStreamIn):
        p0 = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_vapor_pressure_bara), dtype=np.float64)
        p1 = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_vapor_pressure_bara), dtype=np.float64)
        dT = 0.05
        for i, id in enumerate(LiquidStreamIn.vapor_pressure_bara.keys()):
            gas_id = list(LiquidStreamIn.vapor_pressure_bara[id]["Stoch Gas"].keys())[0]
            p0[:, i] = LiquidStreamIn.get_specie_vapor_pressure_bara(gas_id)
        LiquidStreamIn.temp_K = LiquidStreamIn.temp_K + dT
        for i, id in enumerate(LiquidStreamIn.vapor_pressure_bara.keys()):
            gas_id = list(LiquidStreamIn.vapor_pressure_bara[id]["Stoch Gas"].keys())[0]
            p1[:, i] = LiquidStreamIn.get_specie_vapor_pressure_bara(gas_id)
        LiquidStreamIn.temp_K = LiquidStreamIn.temp_K - dT
        return (p1 - p0) / dT
...
class LiquidHeatExchanger_CounterCurrent(__Stochiometry__, __Serializer__, __LiquidEquilibrium__, __StreamFunctions__):
    ...
    def react(self, LiquidStreamIn1, LiquidStreamIn2, volume1_m3, volume2_m3, interface_area_m2, heat_transfer_coefficient_kW_m2K, lr=0.75):

        # Geometry
        self.volume1_m3 = volume1_m3
        self.volume2_m3 = volume2_m3
        ...
        self.LP1 = self.__init_LiquidProfile__(self.LiquidStreamIn1, self.num_of_heights)
        self.LP2 = self.__init_LiquidProfile__(self.LiquidStreamIn2, self.num_of_heights)
    ...
    def get_profile_stream1_temp_K(self):
        pass

    def get_profile_stream2_temp_K(self):
        pass
...
class GasLiquidContactor_CounterCurrent(__Stochiometry__, __Serializer__, __LiquidEquilibrium__, __StreamFunctions__):
    ...
    def react(self, GasStreamIn, LiquidStreamIn, area_m2, height_m, void_fraction_m3_m3):
        ...
        self.LiquidStreamIn = self.__get_LiquidEquilibrium_reduced__(LiquidStreamIn, matrix=self.matrix_liq, lr=0.75)
        ...
        if self.firstscan:
            ...
            # Initiate Liquid and Gas Profile
            self.LP = self.__init_LiquidProfile__(self.LiquidStreamIn, self.num_of_heights)
            self.GP = self.__init_GasProfile__(self.GasStreamIn, self.num_of_heights)


        self.firstscan = False

        # Geometry
        self._area_m2_ = area_m2
        ...
        for _ in range(10):
            ...
            for i in range(self.num_of_heights - 2, -1, -1):
                ...
                self.GasStream = self.__get_GasProfile_slice__(self.GasStream, self.GP, i)
            ...
            for i in range(1, self.num_of_heights, 1):
                ...
                self.LiquidStream = self.__get_LiquidProfile_slice__(self.LiquidStream, self.LP, i)

                # Calculate Gradients
                dydz, dTdz, dndz_tot = self.__get_gradients_gas__(i=i)
                ...
                self.GasStream.flow_kmol_h = self.GasStream.flow_kmol_h + 3600 * dndz_tot * dz
    ...
    def __get_gradients_liq__(self, i):
        ...
        self.area_m2 = self._area_m2_[i+1,:]
        ...
        dmdz_liq = self.area_m2 * (self.M_liq * aJ_specie_liq + self.void_fraction_m3_m3[:, None] * liquid_holdup_m3_m3[:, None] * self.M_liq * r)
    ...
    def __get_gradients_gas__(self, i):
        ...
        dndz = self.area_m2 * (aJ_specie_gas + self.void_fraction_m3_m3[:, None] * (1 - liquid_holdup_m3_m3[:, None]) * r)
    ...
    def get_profile_superficial_gas_velocity_m_s(self, sample_index):
        T = self.GP.get_gas_temp_K()
        n = self.GP.get_gas_flow_kmol_h() / 3600
        p = self.GP.get_gas_pressure_bara()
        R = 0.08314
        V = n * R * T / p
        v = V / self._area_m2_
        return v[:,sample_index], self.z[:, sample_index]

    def get_profile_superficial_liquid_velocity_m_s(self, sample_index):
        rho = self.LP.get_solution_density_kg_m3()
        m = self.LP.get_solution_flow_kg_h() / 3600
        V = m / rho
        v = V / self._area_m2_
        return v[:,sample_index], self.z[:, sample_index]
