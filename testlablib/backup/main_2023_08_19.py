...
class _GasLiquidContactor_PFR:
    ...
    def __get_gradients_liq__(self):
        ...
        dmdz_liq = self._cross_sectional_area_m2 * (self.M_liq * aJ_specie_liq + self._void_fraction_m3_m3 * 0.1 * self.M_liq * r)
        dmdz_liq_tot = self._cross_sectional_area_m2 * np.sum(self.M_liq * aJ_specie_liq, axis=1, keepdims=False)
        dmdz_gas = self._cross_sectional_area_m2 * self.M_gas * aJ_specie_gas
    ...
    def __get_gradients_gas__(self):

        #self.area_m2_slice = self.area_m2[i-1,:]
        #self.void_fraction_m3_m3_slice = self.void_fraction_m3_m3[i-1,:]
...
class GasLiquidContactor_PFR_CounterCurrent(_GasLiquidContactor_PFR ,_Stochiometry, _Serializer, _LiquidEquilibrium, _StreamFunctions):
    ...
    def __react__(self, GasStreamIn, LiquidStreamIn, cross_sectional_area_m2, position_m, void_fraction_m3_m3, max_epochs=np.inf):

        # Geometry
        self._profile_area_m2 = cross_sectional_area_m2
        self._profile_void_fraction_m3_m3 = void_fraction_m3_m3
        self._profile_height_m = position_m
        self._profile_dz = np.diff(self._profile_height_m, axis=0)

        # Various
        self.num_of_heights = cross_sectional_area_m2.shape[0]
        ...
        if self.firstscan:
            ...
            self.LP = self.__broadcast_to_LiquidProfile__(self.LiquidStreamIn, self.num_of_heights)
            self.GP = self.__broadcast_to_GasProfile__(self.GasStreamIn, self.num_of_heights)
        ...
        while converged == False:
            ...
            for i in range(self.num_of_heights - 2, -1, -1):
                ...
                self._dz = self._profile_dz[i, :]
                self._area_m2 = self._profile_area_m2[i + 1, :]
                self._void_fraction_m3_m3 = self._profile_void_fraction_m3_m3[i + 1, :]

                # Load GasStream at height = z[i]
                self.GasStream = self.__get_slice_from_GasProfile__(self.GP, i + 1, self.GasStream)

                # Calculate Gradients
                dwdz, dTdz, dmdz_tot = self.__get_gradients_liq__(i=i)

                # Integrate
                self.LiquidStream.__mass_fractions_vec2dic__(self.LiquidStream.__mass_fractions_dic2vec__() + dwdz * self._dz[:,None])
                self.LiquidStream.temp_K = self.LiquidStream.temp_K + dTdz * self._dz
                self.LiquidStream.flow_kg_h = self.LiquidStream.flow_kg_h + 3600 * dmdz_tot * self._dz
                self.LiquidStream.normalize_mass_fractions()

                # Update Liquid Profile
                self.LP = self.__insert_slice_to_LiquidProfile__(self.LP, i, self.LiquidStream)
            ...
            for i in range(1, self.num_of_heights, 1):
                ...
                self._dz = self._profile_dz[i-1, :]
                self._area_m2 = self._profile_area_m2[i - 1, :]
                self._void_fraction_m3_m3 = self._profile_void_fraction_m3_m3[i - 1, :]

                # Load LiquidStream at height = z[i]
                self.LiquidStream = self.__get_slice_from_LiquidProfile__(self.LP, i-1, self.LiquidStream)

                # Calculate Gradients
                dydz, dTdz, dndz_tot = self.__get_gradients_gas__(i=i)

                # Integrate
                self.GasStream.__molar_fractions_vec2dic__(self.GasStream.__molar_fractions_dic2vec__() + dydz * self._dz[:,None])
                self.GasStream.temp_K = self.GasStream.temp_K + dTdz * self._dz
                self.GasStream.flow_kmol_h = self.GasStream.flow_kmol_h + 3600 * dndz_tot * self._dz
                self.GasStream.normalize_molar_fractions()

                # Update Gas Profile
                self.GP = self.__insert_slice_to_GasProfile__(self.GP, i, self.GasStream)
            ...
            gas_converged = np.min(gas_converged, axis=1, keepdims=False)
            sample_converged = np.minimum(liq_converged, gas_converged)
            sample_converged = sample_converged * (np.abs(dT_gas) < 1.0)
            sample_converged = sample_converged * (np.abs(dT_liq) < 1.0)
            converged = np.min(sample_converged, axis=0)
            w_old = 1.0 * w
            y_old = 1.0 * y
            T_old_liq = 1.0 * T_liq
            T_old_gas = 1.0 * T_gas
            epochs = epochs + 1
            converged = converged or (epochs>=max_epochs)

        print(epochs)
        return self.GasStream, self.LiquidStream


class GasLiquidContactor_PFR_CounterCurrent_Adaptive_Step_Size(_GasLiquidContactor_PFR ,_Stochiometry, _Serializer, _LiquidEquilibrium, _StreamFunctions):
    ...
    def __react__(self, GasStreamIn, LiquidStreamIn, max_epochs=np.inf):
        ...
        # Initiate Profiles
        if self.firstscan:
            self.LP = self.__broadcast_to_LiquidProfile__(LiquidStreamIn=self.LiquidStream, num_of_heights=self.num_of_heights)
            self.GP = self.__broadcast_to_GasProfile__(GasStreamIn=self.GasStream, num_of_heights=self.num_of_heights)

            self.zp_gas = self._profile_position_m
            self.zp_liq = self._profile_position_m
        ...
        while converged == False:
            ...
            z = self._profile_position_m[-1]
            self.zp_liq = [z]
            self.LiquidStream = deepcopy(self.LiquidStreamIn)
            self.LP = deepcopy(self.LiquidStreamIn)

            while z > self._profile_position_m[0]:

                # Interpolate Cross Sectional Area and Void Space at Current Position
                ...
                # Interpolate GasStream at Current Position
                self.GasStream = self.__interpolate_from_GasProfile__(self.GP, self.zp_gas, z, self.GasStream)

                # Calculate Gradients
                dwdz, dTdz, dmdz_tot = self.__get_gradients_liq__()

                # Only Allow Maximum 25% Change in Mass Fractions and 3 Degree Change in Temperature
                mass_frac = self.LiquidStream.__mass_fractions_dic2vec__()
                with np.errstate(divide='ignore', invalid='ignore'):
                    dz_1 = 0.2 * np.min(np.nan_to_num(mass_frac / np.abs(dwdz), nan=np.inf, posinf=np.inf, neginf=np.inf),axis=1, keepdims=False)
                with np.errstate(divide='ignore', invalid='ignore'):
                    dz_2 = 3 * np.nan_to_num(1 / np.abs(dTdz), nan=np.inf, posinf=np.inf, neginf=np.inf)
                dz_3 = z - self._profile_position_m[0]
                dz = np.min(np.vstack((dz_1, dz_2)))      # Scalar Value
                dz = np.minimum(dz, dz_3)

                if z > self._profile_position_m[-1] - 0.2:
                    dz = np.minimum(dz, 0.02)
                else:
                    dz = np.minimum(dz, 0.08)


                # Integrate
                self.LiquidStream.__mass_fractions_vec2dic__(mass_frac + dwdz * dz)
                ...
                self.zp_liq.append(z)

            self.zp_liq = np.array(self.zp_liq)
            ...
            z = self._profile_position_m[0]
            self.zp_gas = [z]

            while z < self._profile_position_m[-1]:

                # Interpolate Cross Sectional Area and Void Space at Current Position
                self._cross_sectional_area_m2 = np.interp(z, self._profile_position_m, self._profile_cross_sectional_area_m2)
                self._void_fraction_m3_m3 = np.interp(z, self._profile_position_m, self._profile_void_fraction_m3_m3)

                # Interpolate LiquidStream at Current Position
                self.LiquidStream = self.__interpolate_from_LiquidProfile__(self.LP, self.zp_liq, z, self.LiquidStream)

                # Calculate Gradients
                dydz, dTdz, dndz_tot = self.__get_gradients_gas__()

                # Only Allow Maximum 25% Change in Molar Fractions and 3 Degree Change in Temperature
                molar_frac = self.GasStream.__molar_fractions_dic2vec__()
                n = self.GasStream.get_gas_flow_kmol_h() / 3600
                with np.errstate(divide='ignore', invalid='ignore'):
                    dz_1 = 0.1 * np.min(np.nan_to_num(molar_frac / np.abs(dydz), nan=np.inf, posinf=np.inf, neginf=np.inf), axis=1,keepdims=False)

                with np.errstate(divide='ignore', invalid='ignore'):
                    dz_2 = 3 * np.nan_to_num(1 / np.abs(dTdz), nan=np.inf, posinf=np.inf, neginf=np.inf)

                with np.errstate(divide='ignore', invalid='ignore'):
                    dz_3 = 0.1 * np.nan_to_num(n / np.abs(dndz_tot), nan=np.inf, posinf=np.inf, neginf=np.inf)

                dz_4 = self._profile_position_m[-1] - z
                dz = np.min(np.vstack((dz_1, dz_2, dz_3)))  # Scalar Value
                dz = np.minimum(dz, dz_4)
                ...
                # Integrate
                self.GasStream.__molar_fractions_vec2dic__(self.GasStream.__molar_fractions_dic2vec__() + dydz * dz)
                ...
                self.zp_gas.append(z)

            self.zp_gas = np.array(self.zp_gas)
...
class _GasLiquidContactor_PFR_RK4:
    ...
    def __f_euler_liq__(self, m, T, w, z):
        ...
        return -dmdz_liq_tot, -dTdz, -dwdz_liq
...
class GasLiquidContactor_PFR_CounterCurrent_RK4(_GasLiquidContactor_PFR_RK4 ,_Stochiometry, _Serializer, _LiquidEquilibrium, _StreamFunctions):
    ...
    def __react__(self, GasStreamIn, LiquidStreamIn, max_epochs=np.inf):
        ...
        while converged == False:
            ...
            while z > self._profile_position_m[0]:
                ...
                m_new, T_new, w_new, z_new = self.__RK1_liq__(m, T, w, z)
            ...
            while z < self._profile_position_m[-1]:
                ...
                n_new, T_new, y_new, z_new = self.__RK1_gas__(n, T, y, z)
...
class GasLiquidContactor_PFR_CoCurrent(_GasLiquidContactor_PFR ,_Stochiometry, _Serializer, _LiquidEquilibrium, _StreamFunctions):
    ...
    def __react__(self, GasStreamIn, LiquidStreamIn, area_m2, height_m, void_fraction_m3_m3):
        ...
        # -----------------------------------------------------------------
        # Integrate GasStream and LiquidStream Downward
        # -----------------------------------------------------------------
        ...
        for i in range(self.num_of_heights - 2, -1, -1):
            ...
            # Calculate Gradients
            dydz, dTdz_gas, dndz_tot = self.__get_gradients_gas__(i=i)
            dwdz, dTdz_liq, dmdz_tot = self.__get_gradients_liq__(i=i)

            # Integrate Gas
            self.GasStream.__molar_fractions_vec2dic__(self.GasStream.__molar_fractions_dic2vec__() + dydz * dz[:, None])
            self.GasStream.temp_K = self.GasStream.temp_K + dTdz_gas * dz
            self.GasStream.flow_kmol_h = self.GasStream.flow_kmol_h + 3600 * dndz_tot * dz
            self.GasStream.normalize_molar_fractions()

            # Integrate Liquid
            self.LiquidStream.__mass_fractions_vec2dic__(self.LiquidStream.__mass_fractions_dic2vec__() + dwdz * dz[:, None])
            self.LiquidStream.temp_K = self.LiquidStream.temp_K + dTdz_liq * dz
            self.LiquidStream.flow_kg_h = self.LiquidStream.flow_kg_h + 3600 * dmdz_tot * dz
            self.LiquidStream.normalize_mass_fractions()
...
class GasLiquidContactor_PFR_GasConstant:
