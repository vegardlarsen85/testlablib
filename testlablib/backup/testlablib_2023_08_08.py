...
class __LiquidEquilibrium__:


    def __get_liquid_equilibrium__(self, LiquidStreamIn, b, matrix, lr):
        ...
        while converged == False:
            ...
            Kw = self.__get_liquid_equilbrium_Kw__(LiquidStreamOut)

            f_rxn_insta = self.__get_liquid_equilibrium_f_rxn_insta__(LiquidStreamIn, Kw)
            f_mass_balance = self.__get_liquid_equilibrium_f_mass_balance__(LiquidStreamIn, b, matrix)
            f = np.concatenate((f_rxn_insta, f_mass_balance), axis=1)

            dfdw_rxn_insta = self.__get_liquid_equilibrium_dfdw_rxn_insta__(LiquidStreamIn, Kw, matrix)
            dfdw_mass_balance = self.__get_liquid_equilibrium_dfdw_mass_balance__(LiquidStreamIn, matrix)
    ...
    def __get_liquid_equilibrium_reduced__(self, LiquidStreamIn, matrix, lr):
        ...
        while converged == False:
            ...
            f = self.__get_liquid_equilibrium_f_rxn_insta__(LiquidStreamIn, Kw)
    ...
    def __get_liquid_equilbrium_Ka__(self, LiquidStreamIn):
    ...
    def __get_liquid_equilbrium_Kw__(self, LiquidStreamIn):
    ...
    def __get_liquid_equilibrium_dKadT__(self, LiquidStreamIn, Ka):
        dT = 0.05
    ...
    def __get_liquid_equilibrium_dKwdT__(self, LiquidStreamIn, Kw):
        dT = 0.05
        T0 = LiquidStreamIn.temp_K
        LiquidStreamIn.temp_K = LiquidStreamIn.temp_K + dT
    ...
    def __get_liquid_equilibrium_exothermic_heat_kJ_kmol_approximation__(self, LiquidStreamIn, Ka, dKadT):
        dT = 0.05
    ...
    def __get_liquid_equilibrium_exothermic_heat_kJ_kmol__(self, LiquidStreamIn, Kw, dKwdT):
        dT = 0.05
        T = LiquidStreamIn.temp_K
        T1 = T + dT
        Kw1 = Kw + dKwdT * dT
    ...
    def __get_liquid_equilibrium_f_rxn_insta__(self, LiquidStreamIn, Kw):
    ...
    def __get_liquid_equilibrium_f_rxn_insta_log__(self, LiquidStreamIn, Kw):
        f_rxn_insta = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_rxn_insta), dtype=np.float64)
        for j, jd in enumerate(LiquidStreamIn.rxn_insta.keys()):
    ...
    def __get_liquid_equilibrium_f_mass_balance__(self, LiquidStreamIn, b, matrix):
        f_mass_balance = np.einsum("cw,sw->sc", matrix["A"], LiquidStreamIn.__mass_fractions_dic2vec__()) - b
        return f_mass_balance

    def __get_liquid_equilibrium_f_energy_balance__(self, LiquidStreamInit, LiquidStreamIn, rxn_insta_exothermic_heat_kJ_kmol, matrix):
    ...
    def __get_liquid_equilibrium_dfdw_rxn_insta__(self, LiquidStreamIn, Kw, matrix):
    ...
    def __get_liquid_equilibrium_dfdw_rxn_insta_loq__(self, LiquidStreamIn):
        dfdw_rxn_insta = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_rxn_insta, LiquidStreamIn.num_of_species),dtype=np.float64)
        for rxn_insta_i, rxn_insta_id in enumerate(LiquidStreamIn.rxn_insta.keys()):
    ...
    def __get_liquid_equilibrium_dfdw_mass_balance__(self, LiquidStreamIn, matrix):
    ...
    def __get_liquid_equilibrium_dfdw_energy_balance__(self, LiquidStreamInit, LiquidStreamIn, rxn_insta_exothermic_heat_kJ_kmol, matrix):
        cp = (LiquidStreamInit.get_solution_heat_capacity_kJ_kgK() + LiquidStreamIn.get_solution_heat_capacity_kJ_kgK()) / 2
    ...
    def __get_liquid_equilibrium_dfdT_rxn_insta__(self, LiquidStreamIn, dKwdT):
    ...
    def __get_liquid_equilibrium_dfdT_mass_balance__(self, LiquidStreamIn):
        dfdT_mass_balance = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0], LiquidStreamIn.num_of_species - LiquidStreamIn.num_of_rxn_insta, 1), dtype=np.float64)
        return dfdT_mass_balance

    def __get_liquid_equilibrium_dfdT_energy_balance__(self, LiquidStreamIn):
        return - np.ones(shape=(LiquidStreamIn.temp_K.shape[0], 1, 1), dtype=np.float64)

    # ----------------------------------------------------------------------------------

    def __get_liquid_sensitivities__(self, LiquidStreamIn, Kw, matrix):

        # Partial Derivatives
        dfdw_rxn_insta = self.__get_liquid_equilibrium_dfdw_rxn_insta_loq__(LiquidStreamIn)
        dfdw_mass_balance = self.__get_liquid_equilibrium_dfdw_mass_balance__(LiquidStreamIn, matrix)
        dfdw = np.concatenate((dfdw_rxn_insta, dfdw_mass_balance), axis=1)
        ...
        dKwdT = self.__get_liquid_equilibrium_dKwdT__(LiquidStreamIn, Kw)
        Hr = H[:, :, :LiquidStreamIn.num_of_rxn_insta:]
        dwdT = - np.einsum("scr,sr->sc", Hr, (1 / Kw) * dKwdT)

        return dwdb, dwdT

    def __get_hessian__(self, LiquidStreamIn, Kw, matrix):
    ...
    def __get_heat_dissipation_kW__(self, LiquidStreamIn, KwIn, dKwdTIn, LiquidStreamOut, KwOut, dKwdTOut, matrix):
...
class GasStream(__Serializer__):
    ...
    def get_rxn_insta_equilibrium_constant_wrt_molar_fractions(self, id):
        ...
        # Kp * p_NO^2 * p_O2 = p_NO2^2
        # Kp * p^2 * p * y_NO^2 * y_O2 = p^2 * y_NO2^2
        # Kp * p * y_NO^2 * y_O2 = y_NO2^2
...
class LiquidStream(__Serializer__):
    ...
    def __compress__(self, condition):
        self.temp_K = np.compress(condition=condition, a=self.temp_K, axis=0)
        self.flow_kg_h = np.compress(condition=condition, a=self.flow_kg_h, axis=0)
        for id in self.specie.keys():
            self.set_specie_mass_fraction(id=id, value=np.compress(condition=condition,a=self.get_specie_mass_fraction(id=id), axis=0))
        for id in self.info.keys():
            self.info[id] = np.compress(condition=condition, a=self.info[id], axis=0)

    def __where__(self, condition, LiquidStreamIn):
        i = np.where(condition)[0]
        self.temp_K[i] = LiquidStreamIn.temp_K
        self.flow_kg_h[i] = LiquidStreamIn.flow_kg_h
        for id in self.specie.keys():
            self.specie[id]["Mass Fraction"][i] = LiquidStreamIn.specie[id]["Mass Fraction"]
        for id in self.info.keys():
            self.info[id][i] = LiquidStreamIn.info[id]

    def __append__(self, LiquidStreamIn):
        self.temp_K = np.concatenate((self.temp_K, LiquidStreamIn.temp_K), axis=0)
        self.flow_kg_h = np.concatenate((self.flow_kg_h, LiquidStreamIn.flow_kg_h), axis=0)
        for id in self.specie.keys():
            self.set_specie_mass_fraction(id=id, value=np.concatenate((self.get_specie_mass_fraction(id=id), LiquidStreamIn.get_specie_mass_fraction(id=id)), axis=0))
        for id in self.info.keys():
            self.info[id] = np.concatenate((self.info[id], LiquidStreamIn.info[id]), axis=0)
...
class LiquidEquilibrium_Isothermal(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
    ...
    def react(self, LiquidStreamIn, lr=0.75):
        ...
        LiquidStreamOut = self.__get_liquid_equilibrium__(LiquidStreamIn, b=None, matrix=self.matrix, lr=lr)
        return LiquidStreamOut


class LiquidEquilibrium_Adiabatic(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
    ...
    def __init__(self):
        ...
        # Various
        self.LiquidStream_Kw = None
        self.LiquidStream_rate = None
        self.LiquidStream_w = None

        # Various, Inlet Stream
        self.LiquidStream_T0 = None
        self.LiquidStream_w0 = None
        self.LiquidStream_b0 = None
        self.LiquidStream_q0 = None
        self.LiquidStream_cp0 = None

        # Constraints
        self.LiquidStream_f_rxn_insta = None
        self.LiquidStream_f_rxn_reversible = None
        self.LiquidStream_f_mass_balance = None
        self.LiquidStream_f_energy_balance = None

        # Partial Derivatives
        self.LiquidStream_dfdw_rxn_insta = None
        self.LiquidStream_dfdw_rxn_reversible = None
        self.LiquidStream_dfdw_mass_balance = None
        self.LiquidStream_dfdw_energy_balance = None

        self.LiquidStream_dfdT_rxn_insta = None
        self.LiquidStream_dfdT_rxn_reversible = None
        self.LiquidStream_dfdT_mass_balance = None
        self.LiquidStream_dfdT_energy_balance = None
    ...
    def react(self, LiquidStreamIn, lr=0.75):
        ...
        if self.firstscan:
            self.__firstscan__(LiquidStreamIn)
            self.__update_inlet__(LiquidStreamIn)
            ...
            while converged == False:
                ...
                # Newton's Method
                self.__f_isothermal__()
                dw_newton = - np.linalg.solve(self.dfdw, self.f)
                dw = lr * dw_newton
                self.LiquidStream_w = self.LiquidStream_w + dw
                self.LiquidStream_w = np.maximum(self.LiquidStream_w, 0)
                self.LiquidStream.__mass_fractions_vec2dic__(self.LiquidStream_w)

                # Check if Algorithm have Converged
                specie_converged = np.array(np.abs(dw_newton) < 0.005 * np.abs(self.LiquidStream_w), dtype=np.float32)
                sample_converged = np.min(specie_converged, axis=1)
                converged = np.min(sample_converged, axis=0)
                converged = (bool(converged) or (epoch > 198)) and (epoch > 0)
                iterations = iterations + (1 - sample_converged)
            ...
            Q = self.get_heat_dissipation_kW()
            cp = 0.5 * (self.LiquidStream_cp0 + self.LiquidStream.get_solution_heat_capacity_kJ_kgK())
            self.LiquidStream.temp_K = self.LiquidStream.temp_K + Q / (self.LiquidStream_m0 * cp)
        ...
        while converged == False:
            self.__f__()
            dX_newton = - np.linalg.solve(self.dfdX, self.f)
            dw_newton = dX_newton[:, :self._num_of_species_:]
            dT_newton = dX_newton[:, self._num_of_species_]
            dw = lr * dw_newton
            dT = lr * dT_newton

            self.LiquidStream_w = self.LiquidStream_w + dw
            self.LiquidStream_w = np.maximum(self.LiquidStream_w, 10 ** (-18))
            self.LiquidStream.__mass_fractions_vec2dic__(self.LiquidStream_w)
            self.LiquidStream.temp_K = self.LiquidStream.temp_K + dT
        ...
        return self.LiquidStream

    def get_heat_dissipation_kW(self):
        q_insta = self.LiquidStream.__exothermic_heat_kJ_kmol_as_vector_rxn_insta__()
        q_reversible = self.LiquidStream.__exothermic_heat_kJ_kmol_as_vector_rxn_reversible__()
        q = np.concatenate((q_insta, q_reversible), axis=1)
        self.q = (self.LiquidStream_q0 + q) / 2
        cp = (self.LiquidStream_cp0 + self.LiquidStream.get_solution_heat_capacity_kJ_kgK()) / 2
        dw = self.LiquidStream_w - self.LiquidStream_w0
        dT = self.LiquidStream.temp_K - self.LiquidStream_T0
        heat_dissipation_kW = self.LiquidStream_m0 * np.einsum("sr,sr->s", self.q, np.einsum("rw,sw->sr", self._matrix_["R+"], dw)) - self.LiquidStream_m0 * cp *dT
        return heat_dissipation_kW

    # ---------------------------------------------------------------

    def __firstscan__(self, LiquidStreamIn):
        R = self.__load_matrix_of_stochiometry_rxn__(LiquidStreamIn)
        self._matrix_ = self.__load_matrix_of_projection_and_conservation__(R)
        self._num_of_rxn_reversible_ = LiquidStreamIn.num_of_rxn_reversible
        self._num_of_rxn_insta_ = LiquidStreamIn.num_of_rxn_insta
        self._num_of_species_ = LiquidStreamIn.num_of_species
        self._num_of_conservations_ = self._num_of_species_ - self._num_of_rxn_insta_ - self._num_of_rxn_reversible_
        self.LiquidStream = deepcopy(LiquidStreamIn)
        self.LiquidStream_w = self.LiquidStream.__mass_fractions_dic2vec__()

    def __update_inlet__(self, LiquidStreamIn):
        self._num_of_samples_ = LiquidStreamIn.temp_K.shape[0]

        A0 = self._matrix_["A"].shape[0]
        A1 = self._matrix_["A"].shape[1]
        R0 = self._matrix_["R+"].shape[0]
        R1 = self._matrix_["R+"].shape[1]
        self._matrix_["A broadcast"] = np.broadcast_to(array=self._matrix_["A"][None, :, :],
                                                       shape=(self._num_of_samples_, A0, A1))
        self._matrix_["R+ broadcast"] = np.broadcast_to(array=self._matrix_["R+"][None, :, :],
                                                        shape=(self._num_of_samples_, R0, R1))

        self.LiquidStream_T0 = LiquidStreamIn.temp_K
        self.LiquidStream_m0 = LiquidStreamIn.get_solution_flow_kg_h() / 3600
        self.LiquidStream_w0 = LiquidStreamIn.__mass_fractions_dic2vec__()
        self.LiquidStream_b0 = np.einsum("cw,sw->sc", self._matrix_["A"], self.LiquidStream_w0)
        q0_insta = LiquidStreamIn.__exothermic_heat_kJ_kmol_as_vector_rxn_insta__()
        q0_reversible = LiquidStreamIn.__exothermic_heat_kJ_kmol_as_vector_rxn_reversible__()
        self.LiquidStream_q0 = np.concatenate((q0_insta, q0_reversible), axis=1)
        self.LiquidStream_cp0 = LiquidStreamIn.get_solution_heat_capacity_kJ_kgK()

    def __f__(self):

        self.__LiquidStream_Kw__()
        self.__LiquidStream_f_rxn_insta__()
        self.__LiquidStream_f_rxn_reversible__()
        self.__LiquidStream_f_mass_balance__()
        self.__LiquidStream_f_energy_balance__()


        self.f = np.hstack((self.LiquidStream_f_rxn_insta, self.LiquidStream_f_rxn_reversible, self.LiquidStream_f_mass_balance, self.LiquidStream_f_energy_balance))

        self.__LiquidStream_dfdw_rxn_insta__()
        self.__LiquidStream_dfdw_rxn_reversible__()
        self.__LiquidStream_dfdw_mass_balance__()
        self.__LiquidStream_dfdw_energy_balance__()

        self.__LiquidStream_dfdT_rxn_insta__()
        self.__LiquidStream_dfdT_rxn_reversible__()
        self.__LiquidStream_dfdT_mass_balance__()
        self.__LiquidStream_dfdT_energy_balance__()

        self.dfdw = np.concatenate((self.LiquidStream_dfdw_rxn_insta, self.LiquidStream_dfdw_rxn_reversible, self.LiquidStream_dfdw_mass_balance, self.LiquidStream_dfdw_energy_balance), axis=1)
        self.dfdT = np.concatenate((self.LiquidStream_dfdT_rxn_insta, self.LiquidStream_dfdT_rxn_reversible, self.LiquidStream_dfdT_mass_balance, self.LiquidStream_dfdT_energy_balance), axis=1)
        self.dfdX = np.concatenate((self.dfdw, self.dfdT), axis=2)

    def __f_isothermal__(self):

        self.__LiquidStream_Kw__()
        self.__LiquidStream_f_rxn_insta__()
        self.__LiquidStream_f_rxn_reversible__()
        self.__LiquidStream_f_mass_balance__()
        self.f = np.hstack((self.LiquidStream_f_rxn_insta, self.LiquidStream_f_rxn_reversible, self.LiquidStream_f_mass_balance))

        self.__LiquidStream_dfdw_rxn_insta__()
        self.__LiquidStream_dfdw_rxn_reversible__()
        self.__LiquidStream_dfdw_mass_balance__()
        self.dfdw = np.concatenate((self.LiquidStream_dfdw_rxn_insta, self.LiquidStream_dfdw_rxn_reversible, self.LiquidStream_dfdw_mass_balance), axis=1)


class LiquidEquilibrium_QPFlash(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
...
class LiquidCSTR_Isothermal(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
    ...
    def react(self, LiquidStreamIn, volume_m3, lr=0.75):
        ...
        if self.firstscan:
            matrix = self.__get_the_matrix__(LiquidStreamIn=LiquidStreamIn, liq_rxn_insta=True, liq_rxn_reversible=True)
            self.firstscan = False

        LiquidStreamIn = deepcopy(LiquidStreamIn)
        for id in LiquidStreamIn.specie.keys():
            LiquidStreamIn.specie[id]["Mass Fraction"] = np.maximum(LiquidStreamIn.specie[id]["Mass Fraction"], 10**(-18))
        LiquidStreamIn.normalize_mass_fractions()
        ...
        b = np.einsum("cw,sw->sc", matrix["A"], LiquidStreamInit.__mass_fractions_dic2vec__())
        w = LiquidStreamIn.__mass_fractions_dic2vec__()

        self.volume_m3 = volume_m3
        phi = LiquidStreamInit.get_solution_flow_kg_h() / (self.volume_m3 * 3600)
        ...
        while converged == False:
            ...
            Kw = self.__get_liquid_equilbrium_Kw__(LiquidStreamIn)
            rates_kmol_m3s = self.__get_CSTR_rxn_reversible_rates_kmol_m3s__(LiquidStreamIn)

            f_rxn_insta = self.__get_liquid_equilibrium_f_rxn_insta__(LiquidStreamIn, Kw)
            f_rxn_reversible = self.__get_CSTR_f_rxn_reversible__(LiquidStreamInit, LiquidStreamIn, matrix, rates_kmol_m3s, phi)
            f_mass_balance = self.__get_liquid_equilibrium_f_mass_balance__(LiquidStreamIn, b, matrix)

            f = np.hstack((f_rxn_insta, f_rxn_reversible, f_mass_balance))

            dfdw_rxn_insta = self.__get_liquid_equilibrium_dfdw_rxn_insta__(LiquidStreamIn, Kw, matrix)
            dfdw_rxn_reversible = self.__get_CSTR_dfdw_rxn_reversible__(LiquidStreamIn, matrix, rates_kmol_m3s, phi)
            dfdw_mass_balance = self.__get_liquid_equilibrium_dfdw_mass_balance__(LiquidStreamIn, matrix)
            ...
            w = w + dw
            w = np.maximum(w, 0)
            LiquidStreamIn.__mass_fractions_vec2dic__(w)
    ...
    def get_heat_dissipation_kW(self):
        ...
        cp = (self.LiquidStream_cp0 + self.LiquidStream.get_solution_heat_capacity_kJ_kgK()) / 2
        dw = self.LiquidStream_w - self.LiquidStream_w0
        dT = self.LiquidStream.temp_K - self.LiquidStreamIn.temp_K
        heat_dissipation_kW = self.LiquidStream_m0 * np.einsum("sr,sr->s", self.q, np.einsum("rw,sw->sr", self._matrix_["R+"], dw)) - self.LiquidStream_m0 * cp *dT
        return heat_dissipation_kW

    def __get_CSTR_rxn_reversible_rates_kmol_m3s__(self, LiquidStreamIn):
    ...
    def __get_CSTR_f_rxn_reversible__(self, LiquidStreamInit, LiquidStreamIn, matrix, rates_kmol_m3s, phi):
    ...
    def __get_CSTR_dfdw_rxn_reversible__(self, LiquidStreamIn, matrix, rates_kmol_m3s, phi):
...
class LiquidCSTR_Adiabatic(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):

    def __init__(self):
        pass


class LiquidCSTR_QPFlash(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
...
class LiquidHeatExchanger_CounterCurrent(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):

    def __init__(self):
        pass


class LiquidHeatExchanger_CounterCurrent_NTU(__Serializer__):
...
class GasCompressor_Isentropic(__Serializer__):
...
class GasLiquidContactor_CounterCurrent(__Stochiometry__, __Serializer__, __LiquidEquilibrium__):
    ...
    def react(self, GasStreamIn, LiquidStreamIn, area_m2, height_m, void_fraction_m3_m3):
        ...
        if self.firstscan:
            ...
            self.matrix_liq = self.__get_the_matrix__(LiquidStreamIn=LiquidStreamIn,
                                                      liq_rxn_insta=True)

            self.matrix_gas = self.__get_the_matrix__(GasStreamIn=GasStreamIn,
                                                      gas_rxn_insta=True)
        ...
        #self.LiquidStreamIn = self.__get_liquid_equilibrium__(LiquidStreamIn, b=None, matrix=self.matrix_liq, lr=0.75)
        self.LiquidStreamIn = self.__get_liquid_equilibrium_reduced__(LiquidStreamIn, matrix=self.matrix_liq, lr=0.75)
        ...
        if self.firstscan:
            ...
            # Initiate Liquid Profile (LP)
            self.LP = deepcopy(self.LiquidStreamIn)
            self.LP.temp_K = self.LP.temp_K[None, :]
            self.LP.temp_K = np.broadcast_to(array=self.LP.temp_K, shape=(self.num_of_heights, self.num_of_samples)).copy()
            self.LP.flow_kg_h = self.LP.flow_kg_h[None, :]
            self.LP.flow_kg_h = np.broadcast_to(array=self.LP.flow_kg_h, shape=(self.num_of_heights, self.num_of_samples)).copy()
            for id in self.LP.specie.keys():
                self.LP.specie[id]["Mass Fraction"] = self.LP.specie[id]["Mass Fraction"][None, :]
                self.LP.specie[id]["Mass Fraction"] = np.broadcast_to(array=self.LP.specie[id]["Mass Fraction"],shape=(self.num_of_heights,self.num_of_samples)).copy()
            for id in self.LP.info.keys():
                self.LP.info[id] = self.LP.info[id][None, :]
                self.LP.info[id] = np.broadcast_to(array=self.LP.info[id],shape=(self.num_of_heights, self.num_of_samples)).copy()

            # Initiate Gas Profile (GP)
            self.GP = deepcopy(self.GasStreamIn)
            self.GP.temp_K = self.GP.temp_K[None, :]
            self.GP.temp_K = np.broadcast_to(array=self.GP.temp_K, shape=(self.num_of_heights, self.num_of_samples)).copy()
            self.GP.flow_kmol_h = self.GP.flow_kmol_h[None, :]
            self.GP.flow_kmol_h = np.broadcast_to(array=self.GP.flow_kmol_h, shape=(self.num_of_heights, self.num_of_samples)).copy()
            for id in self.GP.specie.keys():
                self.GP.specie[id]["Molar Fraction"] = self.GP.specie[id]["Molar Fraction"][None, :]
                self.GP.specie[id]["Molar Fraction"] = np.broadcast_to(array=self.GP.specie[id]["Molar Fraction"],shape=(self.num_of_heights,self.num_of_samples)).copy()
            for id in self.GP.info.keys():
                self.GP.info[id] = self.GP.info[id][None, :]
                self.GP.info[id] = np.broadcast_to(array=self.GP.info[id],shape=(self.num_of_heights, self.num_of_samples)).copy()
        ...
        self.z = np.zeros(shape=(self.num_of_heights, self.num_of_samples))
        for i in range(self.num_of_samples):
            self.z[:, i] = np.linspace(0, self.height_m[i], self.num_of_heights)
        dz = self.height_m / self.num_of_heights
        ...
        for _ in range(10):
            ...
            for i in range(self.num_of_heights - 2, -1, -1):
                ...
                # Load GasStream at height = z[i]
                self.GasStream.temp_K = self.GP.temp_K[i, :]
                self.GasStream.flow_kmol_h = self.GP.flow_kmol_h[i, :]
                for id in self.GasStream.specie.keys():
                    self.GasStream.specie[id]["Molar Fraction"] = self.GP.specie[id]["Molar Fraction"][i, :]
                for id in self.GasStream.info.keys():
                    self.GasStream.info[id] = self.GP.info[id][i, :]

                # Calculate Gradients
                dwdz, dTdz, dmdz_tot = self.__get_gradients_liq__(i=i)

                # Integrate
                self.LiquidStream.__mass_fractions_vec2dic__(self.LiquidStream.__mass_fractions_dic2vec__() + dwdz * dz[:, None])
                self.LiquidStream.temp_K = self.LiquidStream.temp_K + dTdz * dz
                self.LiquidStream.flow_kg_h = self.LiquidStream.flow_kg_h + 3600 * dmdz_tot * dz
                self.LiquidStream.normalize_mass_fractions()

                # Update Liquid Profile
                self.LP.flow_kg_h[i, :] = self.LiquidStream.flow_kg_h
                self.LP.temp_K[i, :] = self.LiquidStream.temp_K
                for id in self.LP.specie.keys():
                    self.LP.specie[id]["Mass Fraction"][i, :] = self.LiquidStream.specie[id]["Mass Fraction"]
                for id in self.LP.info.keys():
                    self.LP.info[id][i, :] = self.LiquidStream.info[id]
            ...
            for i in range(1, self.num_of_heights, 1):

                # Load LiquidStream at height = z[i]
                self.LiquidStream.temp_K = self.LP.temp_K[i, :]
                self.LiquidStream.flow_kg_h = self.LP.flow_kg_h[i, :]
                for id in self.LiquidStream.specie.keys():
                    self.LiquidStream.specie[id]["Mass Fraction"] = self.LP.specie[id]["Mass Fraction"][i, :]
                for id in self.LiquidStream.info.keys():
                    self.LiquidStream.info[id] = self.LP.info[id][i, :]

                # Calculate Gradients
                dydz, dTdz, dndz = self.__get_gradients_gas__(i=i)

                # Integrate
                self.GasStream.__molar_fractions_vec2dic__(self.GasStream.__molar_fractions_dic2vec__() + dydz * dz[:, None])
                self.GasStream.temp_K = self.GasStream.temp_K + dTdz * dz
                #self.GasStream.flow_kmol_h = self.GasStream.flow_kmol_h + 3600 * dndz * dz
                self.GasStream.normalize_molar_fractions()

                # Update Profile
                self.GP.flow_kmol_h[i, :] = self.GasStream.flow_kmol_h
                self.GP.temp_K[i, :] = self.GasStream.temp_K
                for id in self.GP.specie.keys():
                    self.GP.specie[id]["Molar Fraction"][i, :] = self.GasStream.specie[id]["Molar Fraction"]
                for id in self.GP.info.keys():
                    self.GP.info[id][i, :] = self.GasStream.info[id]

        return self.GasStream, self.LiquidStream

    # --------------------------------------------------------------------

    def __get_gradients_liq__(self, i):
        ...
        Kw = self.__get_liquid_equilbrium_Kw__(LiquidStreamIn = self.LiquidStream)
        dwdb, dwdT = self.__get_liquid_sensitivities__(self.LiquidStream, Kw, self.matrix_liq)
        ...
        dmdz_liq = self.area_m2 * (self.M_liq * aJ_specie_liq + self.void_fraction_m3_m3[:, None] * liquid_holdup_m3_m3 * self.M_liq * r)
        dmdz_liq_tot = self.area_m2 * np.sum(self.M_liq * aJ_specie_liq, axis=1, keepdims=False)
        dmdz_gas = self.area_m2 * self.M_gas * aJ_specie_gas
        dmdz_gas_tot = - dmdz_liq_tot

        # Quotient Rule
        dwdz_liq = (dmdz_liq * m_liq_tot - m_liq * dmdz_liq_tot) / m_liq_tot ** 2
        dwdz_gas = (dmdz_gas * m_gas_tot - m_gas * dmdz_gas_tot) / m_gas_tot ** 2
        ...
        for _ in range(2):
            ...
            dmdz_liq = (dwdz_liq * m_liq_tot ** 2 + m_liq * dmdz_liq_tot) / m_liq_tot
            ...
            q_lat = (m_gas_tot + m_liq_tot) * np.einsum("sr,sr->s", h, drdz) / self.area_m2  # [kW/m] = [kg/s] [kJ/kmol] [kmol/kg.m] * [1/m2]
        ...
        return dwdz_liq, dTdz, dmdz_liq_tot

    def __get_gradients_gas__(self, i):
        ...
        # Calculate Hessian and Sensitivity Matrix (dw/db)
        #Ky = self.__get_liquid_equilbrium_Kw__(LiquidStreamIn=self.LiquidStream)
        #dwdb, dwdT = self.__get_liquid_sensitivities__(self.LiquidStream, Kw)
        ...
        dndz = self.area_m2 * (aJ_specie_gas + self.void_fraction_m3_m3[:, None] * (1 - liquid_holdup_m3_m3) * r)
        dndz_tot = np.sum(dndz, axis=1, keepdims=False)

        # Quotient Rule
        dydz = (dndz * n_tot - n * dndz_tot) / n_tot ** 2
        ...
        dTdz = ((R * T_gas) / (p_gas * cp * v_GS)) * q_gas

        return dydz, dTdz, dndz
...
class OLD_LiquidReactor_Equilibrium_with_Flash(__Serializer__):
...
class OLD_CSTR_FlashEvaporation(__Serializer__):

    def __init__(self):
        pass


class OLD_Equilibrium_Gas_TP(__Serializer__):
...
class OLD_Equilibrium_VaporLiquid(__Serializer__):
...
class OLD___Reactor_Common_Old__(__Serializer__):

    def __init__(self):
        super().__init__()
        self.w_min_init = 10**(-18)
        self.w_min = 10**(-18)
        self.dw_min_td = 10**(-14)

    def __load_matrix_of_stochiometry_rxn__(self, StreamIn):

        num_of_species = len(StreamIn.specie.keys())
        num_of_rxn_insta = len(StreamIn.rxn_insta.keys())
        num_of_rxn_reversible = len(StreamIn.rxn_reversible.keys())
        num_of_conserved_quantities = num_of_species - num_of_rxn_insta - num_of_rxn_reversible

        R_insta = np.zeros(shape=(num_of_species, num_of_rxn_insta), dtype=np.float64)
        for j, jd in enumerate(StreamIn.rxn_insta.keys()):
            for id in StreamIn.specie.keys():
                i = StreamIn.specie[id]["Index"]
                if id in StreamIn.rxn_insta[jd]["Stoch"].keys():
                    nu = StreamIn.rxn_insta[jd]["Stoch"][id]
                    R_insta[i, j] = nu * StreamIn.get_specie_molar_mass_kg_kmol(id=id)

        R_reversible = np.zeros(shape=(num_of_species, num_of_rxn_reversible), dtype=np.float64)
        for j, jd in enumerate(StreamIn.rxn_reversible.keys()):
            for id in StreamIn.specie.keys():
                i = StreamIn.specie[id]["Index"]
                if id in StreamIn.rxn_reversible[jd]["Stoch"].keys():
                    nu = StreamIn.rxn_reversible[jd]["Stoch"][id]
                    R_reversible[i, j] = nu * StreamIn.get_specie_molar_mass_kg_kmol(id=id)

        R = np.concatenate((R_insta, R_reversible), axis=1)
        return R

    def __load_matrix_of_stochiometry_rxn_and_vapor__(self, GasStreamIn, LiquidStreamIn):

        R_liq = self.__load_matrix_of_stochiometry_rxn__(LiquidStreamIn)
        R_gas = self.__load_matrix_of_stochiometry_rxn__(GasStreamIn)

        num_of_species_liq = len(LiquidStreamIn.specie.keys())
        num_of_species_gas = len(GasStreamIn.specie.keys())
        num_of_vapor = len(LiquidStreamIn.vapor_pressure_bara.keys())
        R_vap_liq = np.zeros(shape=(num_of_species_liq, num_of_vapor), dtype=np.float64)
        R_vap_gas = np.zeros(shape=(num_of_species_gas, num_of_vapor), dtype=np.float64)
        ...
        for j, jd in enumerate(LiquidStreamIn.vapor_pressure_bara.keys()):
            for id in LiquidStreamIn.specie.keys():
                ...
                if id in LiquidStreamIn.vapor_pressure_bara[jd]["Stoch Liq"].keys():
                    nu = LiquidStreamIn.vapor_pressure_bara[jd]["Stoch Liq"][id]
                    R_vap_liq[i, j] = nu * LiquidStreamIn.get_specie_molar_mass_kg_kmol(id=id)
        ...
        for j, jd in enumerate(LiquidStreamIn.vapor_pressure_bara.keys()):
            for id in GasStreamIn.specie.keys():
                ...
                if id in LiquidStreamIn.vapor_pressure_bara[jd]["Stoch Gas"].keys():
                    nu = LiquidStreamIn.vapor_pressure_bara[jd]["Stoch Gas"][id]
                    R_vap_gas[i, j] = nu * GasStreamIn.get_specie_molar_mass_kg_kmol(id=id)

        Z0 = np.zeros(shape=(R_liq.shape[0], R_gas.shape[1]), dtype=np.float64)
        Z1 = np.zeros(shape=(R_gas.shape[0], R_liq.shape[1]), dtype=np.float64)
        ...
        R1 = np.concatenate((R_liq, R_vap_liq, Z0), axis=1)
        R2 = np.concatenate((Z1, R_vap_gas, R_gas), axis=1)
        R = np.concatenate((R1, R2), axis=0)
        return R

    def __load_matrix_of_stochiometry_rxn_and_mass_transfer__(self, GasStreamIn, LiquidStreamIn):
        pass

    def __load_matrix_of_projection_and_conservation__(self, R):
        matrix = {}
        matrix["R"] = R
        matrix["R+"] = np.linalg.pinv(matrix["R"]).astype(np.float64)
        matrix["P"] = np.einsum("cq,dq->cd", np.einsum("cr,rq->cq", matrix["R"], np.linalg.pinv(np.einsum("cr,cq->rq", matrix["R"], matrix["R"]))), matrix["R"]).astype(np.float64)
    ...
    def __get_Equilibrium_Liquid_energy_balance_f__(self):
        pass

    # --------------------------------------------------------------------------

    def __get_equilibrium_liq_dldw_failed__(self, LiquidStreamIn, K):
        num_of_samples = LiquidStreamIn.temp_K.shape[0]
    ...
    def __get_equilibrium_liq_dldw__(self, LiquidStreamIn, K):
        num_of_samples = LiquidStreamIn.temp_K.shape[0]
        num_of_equilibriums = LiquidStreamIn.num_of_rxn_insta
        num_of_species = LiquidStreamIn.num_of_species
        l = self.__get_equilibrium_liq_l__(LiquidStreamIn, K)
        dldw = np.zeros(shape=(num_of_samples, num_of_equilibriums, num_of_species), dtype=np.float64)
        for specie_i, specie_id in enumerate(LiquidStreamIn.specie.keys()):
            w = LiquidStreamIn.get_specie_mass_fraction(id=specie_id)
            dw = np.clip(a=0.01 * w, a_min=self.dw_min_td, a_max=10**(-3))
            LiquidStreamIn.set_specie_mass_fraction(id=specie_id, value=w + dw)
            l_new = self.__get_equilibrium_liq_l__(LiquidStreamIn, K)
    ...
    def __get_equilibrium_vap_K__(self, GasStreamIn, LiquidStreamIn):
        ...
        for i, id in enumerate(d.keys()):
            ...
            if d[id]["H"] is None:
                K[:, i] = 1 / d[id]["p0"](LiquidStreamIn)
            else:
                K[:, i] = d[id]["H"](LiquidStreamIn)
...
class OLD_LiquidStream_Equilibrium_Isothermal_Insta_Only_with_kNN(__Stochiometry__, __Serializer__):

    def __init__(self):
        self._firstscan_ = True
        self._first_training_iteration_ = True

    def react(self, LiquidStreamIn, lr=0.75, max_extrapolation_length=1.5):
        ...
        if self._firstscan_:
            self._matrix_ = self.__load_matrix_of_projection_and_conservation__(R=self.__load_matrix_of_stochiometry_rxn_insta__(LiquidStreamIn))
            self._firstscan_ = False

        # Calculate Approximate Solution using First Order Taylor Expansion
        if self._first_training_iteration_:
            LiquidStreamApprox = deepcopy(LiquidStreamIn)
            sample_accurate = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0],), dtype=bool)
            sample_valid = np.zeros(shape=(LiquidStreamIn.temp_K.shape[0],), dtype=bool)
        else:
            LiquidStreamApprox, sample_accurate, sample_valid = self.__approximate__(LiquidStreamIn, max_extrapolation_length)

        # If Approximation is Valid Then set self.LiquidStream equal to approx. Value Else set it equal to Inlet
        LiquidStreamApproxValid = deepcopy(LiquidStreamApprox)
        LiquidStreamApproxValid.__compress__(condition=sample_valid)
        self.LiquidStream = deepcopy(LiquidStreamIn)
        self.LiquidStream.__where__(condition=sample_valid, LiquidStreamIn=LiquidStreamApproxValid)

        # Extract Samples where the Approximator is Inaccurate and Need to be Re-Calculated
        self.LiquidStream.__compress__(condition=np.invert(sample_accurate))
        self._num_of_samples_ = self.LiquidStream.temp_K.shape[0]
        self._num_of_rxn_reversible_ = self.LiquidStream.num_of_rxn_reversible
        self._num_of_rxn_insta_ = self.LiquidStream.num_of_rxn_insta
        self._num_of_species_ = self.LiquidStream.num_of_species
        self._num_of_conservations_ = self._num_of_species_ - self._num_of_rxn_insta_ - self._num_of_rxn_reversible_

        epoch = 0
        if self._num_of_samples_ == 0:
            LiquidStreamOut = deepcopy(LiquidStreamApprox)
        else:
            w = self.LiquidStream.__mass_fractions_dic2vec__()
            w0 = self.LiquidStream.__mass_fractions_dic2vec__()
            ...
            while converged == False:
                ...
                Kw = self.__Kw__(self.LiquidStream)

                # Objective Function
                f = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_insta_),dtype=np.float64)
                for j, jd in enumerate(self.LiquidStream.rxn_insta.keys()):
                    fr = Kw[:, j]
                    fp = np.ones(shape=(self._num_of_samples_,), dtype=np.float64)
                    for id in self.LiquidStream.rxn_insta[jd]["Stoch"].keys():
                        nu = self.LiquidStream.rxn_insta[jd]["Stoch"][id]
                        if nu > 0:
                            fp = fp * self.LiquidStream.get_specie_mass_fraction(id=id) ** np.abs(nu)
                        else:
                            fr = fr * self.LiquidStream.get_specie_mass_fraction(id=id) ** np.abs(nu)
                    f[:, j] = fr - fp

                # Partial Derivatives
                dfdw = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_insta_, self._num_of_species_), dtype=np.float64)
                for rxn_insta_i, rxn_insta_id in enumerate(self.LiquidStream.rxn_insta.keys()):
                    for specie_i, specie_id in enumerate(self.LiquidStream.specie.keys()):
                        if specie_id in list(self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys()):
                            nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_id]
                            if nu < 0:
                                dfdw[:, rxn_insta_i, specie_i] = Kw[:,rxn_insta_i] * np.abs(nu) * self.LiquidStream.get_specie_mass_fraction(id=specie_id) ** (np.abs(nu) - 1)
                                for specie_jd in self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys():
                                    nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_jd]
                                    if specie_id != specie_jd and nu < 0:
                                        dfdw[:, rxn_insta_i,specie_i] = dfdw[:, rxn_insta_i,specie_i] * self.LiquidStream.get_specie_mass_fraction(id=specie_jd) ** np.abs(nu)
                            else:
                                dfdw[:, rxn_insta_i, specie_i] = - np.abs(nu) * self.LiquidStream.get_specie_mass_fraction(id=specie_id) ** (np.abs(nu) - 1)
                                for specie_jd in self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys():
                                    nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_jd]
                                    if specie_id != specie_jd and nu > 0:
                                        dfdw[:, rxn_insta_i,specie_i] = dfdw[:, rxn_insta_i,specie_i] * self.LiquidStream.get_specie_mass_fraction(id=specie_jd) ** np.abs(nu)
                dfdr = np.einsum("sfw,wr->sfr", dfdw, self._matrix_["R"])

                # Newton's Method
                dr_newton = - np.linalg.solve(dfdr, f)
                dw_newton = np.einsum("wr,sr->sw", self._matrix_["R"], dr_newton)
                dr = lr * dr_newton
                dw = lr * dw_newton
                ...
                w = w0 + np.einsum("wr,sr->sw", self._matrix_["R"], r)
                w = np.maximum(w, 0)
            ...
            self.__train__(LiquidStreamIn=self.LiquidStream)

            LiquidStreamOut = deepcopy(LiquidStreamApprox)
            LiquidStreamOut.__where__(condition=np.invert(sample_accurate), LiquidStreamIn=self.LiquidStream)

        print("Epochs: \t" + str(epoch))

        q_in = LiquidStreamIn.__exothermic_heat_kJ_kmol_as_vector_rxn_insta__()
        q_out = LiquidStreamOut.__exothermic_heat_kJ_kmol_as_vector_rxn_insta__()
        q = (q_in + q_out) / 2
        cp_in = LiquidStreamIn.get_solution_heat_capacity_kJ_kgK()
        cp_out = LiquidStreamOut.get_solution_heat_capacity_kJ_kgK()
        cp = (cp_in + cp_out) / 2
        dw = LiquidStreamOut.__mass_fractions_dic2vec__() - LiquidStreamIn.__mass_fractions_dic2vec__()
        dT = LiquidStreamOut.temp_K - LiquidStreamIn.temp_K
        m = LiquidStreamIn.flow_kg_h / 3600
        heat_dissipation_kW = m * np.einsum("sr,sr->s", q, np.einsum("rw,sw->sr", self._matrix_["R+"], dw)) - m * cp * dT

        return LiquidStreamOut, heat_dissipation_kW

    def __approximate__(self, LiquidStreamIn, max_extrapolation_length):

        # Approx Mass Fractions (at Equilibrium)
        T_new = LiquidStreamIn.get_solution_temp_K()
        w_new = LiquidStreamIn.__mass_fractions_dic2vec__()
        b_new = np.einsum("ic,sc->si", self._matrix_["A"], w_new).astype(np.float64)
        ...
        not_increased_to_much = (w_new < max_extrapolation_length * w)
        not_decreased_to_much = (w_new > w / max_extrapolation_length)
        dont_care = (w <= 10**(-18))
        ...
        for id in LiquidStreamIn.specie.keys():
            i = LiquidStreamIn.specie[id]["Index"]
            LiquidStreamOut.set_specie_mass_fraction(id=id, value=np.array(np.maximum(w_new[:, i], 0.0), dtype=np.float32))
        LiquidStreamOut.normalize_mass_fractions()

        return LiquidStreamOut, sample_accurate, sample_valid

    def __train__(self, LiquidStreamIn):

        LiquidStreamTrain = deepcopy(LiquidStreamIn)

        if LiquidStreamTrain.temp_K.shape[0] > 0:

            # Partial Derivatives
            dfdw_rxn_insta = np.zeros(shape=(LiquidStreamTrain.temp_K.shape[0], LiquidStreamTrain.num_of_rxn_insta, LiquidStreamTrain.num_of_species), dtype=np.float64)
            for rxn_insta_i, rxn_insta_id in enumerate(LiquidStreamTrain.rxn_insta.keys()):
                for _, specie_id in enumerate(LiquidStreamTrain.rxn_insta[rxn_insta_id]["Stoch"].keys()):
                    specie_i = LiquidStreamTrain.specie[specie_id]["Index"]
                    nu = LiquidStreamTrain.rxn_insta[rxn_insta_id]["Stoch"][specie_id]
                    dfdw_rxn_insta[:, rxn_insta_i, specie_i] = - nu / LiquidStreamTrain.get_specie_mass_fraction(id=specie_id)
            A0 = self._matrix_["A"].shape[0]
            A1 = self._matrix_["A"].shape[1]
            dfdw_mass_balance = np.broadcast_to(array=self._matrix_["A"][None, :, :], shape=(LiquidStreamTrain.temp_K.shape[0], A0, A1))
            dfdw = np.concatenate((dfdw_rxn_insta, dfdw_mass_balance), axis=1)
            ...
            # Sensitivity Matrix (dw/db) with respect to "b"
            dwdb = H[:, :, LiquidStreamTrain.num_of_rxn_insta::]

            # Sensitivity Matrix (dw/dT) with respect to "T"
            Kw = self.__Kw__(LiquidStreamTrain)
            LiquidStreamTrain.temp_K = LiquidStreamTrain.temp_K + 0.1
            Kw2 = self.__Kw__(LiquidStreamTrain)
            LiquidStreamTrain.temp_K = LiquidStreamTrain.temp_K - 0.1
            dKwdT = (Kw2 - Kw) / 0.1
            Hr = H[:, :, :LiquidStreamTrain.num_of_rxn_insta:]
            dwdT = - np.einsum("scr,sr->sc", Hr, (1 / Kw) * dKwdT)[:, :, None]

            # Store Samples in Info Dictionary
            w = LiquidStreamTrain.__mass_fractions_dic2vec__()
            LiquidStreamTrain.info["dwdb"] = dwdb
            LiquidStreamTrain.info["dwdT"] = dwdT
            LiquidStreamTrain.info["b"] = np.einsum("ic,sc->si", self._matrix_["A"], w).astype(np.float64)
            LiquidStreamTrain.info["T"] = LiquidStreamTrain.get_solution_temp_K()[:, None]
            LiquidStreamTrain.info["w"] = w

            # Save for later...
            if self._first_training_iteration_:
                self.LiquidStreamTarget = deepcopy(LiquidStreamTrain)
            else:
                self.LiquidStreamTarget.__append__(LiquidStreamTrain)

            # Dimensional Reduction of (b, dw/db) into (p, dw/dp) using Principal Component Analysis
            ...
            # Combining "p" and "T" into the vector "x" using scaling factor "G".
            G = np.clip(a=np.mean(np.abs(self.LiquidStreamTarget.info["dwdT"])) / np.mean(np.abs(dwdp)), a_min=10 ** (-12), a_max=1.0)
...
class RecycleBin:
    ...
    # Not in Use
    def __get_liquid_equilibrium_dfdT_rxn_reversible__(self):

        self.LiquidStream_dfdT_rxn_reversible = np.zeros(
            shape=(self._num_of_samples_, self._num_of_rxn_reversible_, 1), dtype=np.float64)
        for rxn_i, rxn_id in enumerate(self.LiquidStream.rxn_reversible.keys()):
            self.LiquidStream.temp_K = self.LiquidStream.temp_K + 0.1
            forward_rate_pertubation, backward_rate_pertubation = self.LiquidStream.rxn_reversible[rxn_id][
                "Rate [kmol/m3.s]"](self.LiquidStream)
            rate_pertubation = forward_rate_pertubation - backward_rate_pertubation
            self.LiquidStream.temp_K = self.LiquidStream.temp_K - 0.1
            self.LiquidStream_dfdT_rxn_reversible[:, rxn_i, 0] = (rate_pertubation - self.LiquidStream_rate[:,
                                                                                     rxn_i]) / 0.1


class OLD_LiquidStream_CSTR_Isothermal_Dynamic(__Stochiometry__, __Serializer__):
    ...
    def react(self, LiquidStreamIn, volume_m3, t):

        self.volume = volume_m3

        R = self.__load_matrix_of_stochiometry_rxn__(LiquidStreamIn)
        R_rev = self.__load_matrix_of_stochiometry_rxn_reversible__(LiquidStreamIn)
        R_insta = self.__load_matrix_of_stochiometry_rxn_insta__(LiquidStreamIn)

        self._matrix_ = self.__load_matrix_of_projection_and_conservation__(R=R)
        self._matrix_rev_ = self.__load_matrix_of_projection_and_conservation__(R=R_rev)
        self._matrix_insta_ = self.__load_matrix_of_projection_and_conservation__(R=R_insta)

        self._num_of_time_steps_ = len(t)
        self._num_of_samples_ = LiquidStreamIn.temp_K.shape[0]
        self._num_of_rxn_reversible_ = LiquidStreamIn.num_of_rxn_reversible
        self._num_of_rxn_insta_ = LiquidStreamIn.num_of_rxn_insta
        self._num_of_species_ = LiquidStreamIn.num_of_species
        ...
        w = np.zeros(shape=(self._num_of_samples_, self._num_of_time_steps_, self._num_of_species_), dtype=np.float64)
        for self.sample_index in range(self._num_of_samples_):

            # Inlet...
            w0 = np.zeros(shape=(self._num_of_species_), dtype=np.float64)
            for specie_i, specie_id in enumerate(LiquidStreamIn.specie.keys()):
                w0[specie_i] = LiquidStreamIn.get_specie_mass_fraction(id=specie_id)[self.sample_index]

            # Constants
            m = np.array([self.LiquidStreamIn.get_solution_flow_kg_h()[self.sample_index]], dtype=np.float64)
            T = np.array([self.LiquidStreamIn.get_solution_temp_K()[self.sample_index]], dtype=np.float64)
            self.LiquidStream.set_solution_flow_kg_h(value=m)
            self.LiquidStream.set_solution_temp_K(value=T)

            # Abra Kadabra
            sol = odeint(self.__dwdt__, w0, t)
            w[self.sample_index,:,:] = sol

            self.__dwdt__(w=w0, t=t)


        for i in range(self._num_of_species_):
            plt.plot(w[2,:,i])
        plt.yscale("log")
        plt.grid(True)
        plt.show()
    ...
    def __LiquidStream_Kw__(self):
        self.LiquidStream_Kw = np.zeros(shape=(1, self._num_of_rxn_insta_), dtype=np.float64)
        for j, jd in enumerate(self.LiquidStream.rxn_insta.keys()):
            self.LiquidStream_Kw[:,j] = self.LiquidStream.get_rxn_insta_equilibrium_constant_wrt_mass_fractions(jd)

    def __LiquidStream_f_rxn_insta__(self):
        self.LiquidStream_f_rxn_insta = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_insta_), dtype=np.float64)
        for j, jd in enumerate(self.LiquidStream.rxn_insta.keys()):
            fr = self.LiquidStream_Kw[:,j]
            fp = np.ones(shape=(self._num_of_samples_,), dtype=np.float64)
            for id in self.LiquidStream.rxn_insta[jd]["Stoch"].keys():
                nu = self.LiquidStream.rxn_insta[jd]["Stoch"][id]
                if nu > 0:
                    ...
                    fr = fr * self.LiquidStream.get_specie_mass_fraction(id=id) ** np.abs(nu)
            self.LiquidStream_f_rxn_insta[:, j] = fr - fp

    def __LiquidStream_f_rxn_reversible__(self):
        self.LiquidStream_rate = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_reversible_), dtype=np.float64)
        for i, id in enumerate(self.LiquidStream.rxn_reversible.keys()):
            r_forward, r_backward = self.LiquidStream.rxn_reversible[id]["Rate [kmol/m3.s]"](self.LiquidStream)
            self.LiquidStream_rate[:, i] = r_forward - r_backward
        RI = self._matrix_["R+"][self._num_of_rxn_insta_::, :]
        dw = self.LiquidStream_w - self.LiquidStream_w0
        self.LiquidStream_f_rxn_reversible = self.LiquidStream_rate - self.phi[:,None] * np.einsum("rw,sw->sr", RI, dw)

    def __LiquidStream_f_mass_balance__(self):
        self.LiquidStream_f_mass_balance = np.einsum("cw,sw->sc", self._matrix_["A"], self.LiquidStream_w - self.LiquidStream_w0)

    # ---------------------------------------------------------------

    def __LiquidStream_dfdw_rxn_insta__(self):

        self.LiquidStream_dfdw_rxn_insta = np.zeros(shape=(1, self._num_of_rxn_insta_, self._num_of_species_), dtype=np.float64)

        for rxn_insta_i, rxn_insta_id in enumerate(self.LiquidStream.rxn_insta.keys()):
            for specie_i, specie_id in enumerate(self.LiquidStream.specie.keys()):
                if specie_id in list(self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys()):
                    nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_id]
                    w = self.LiquidStream.get_specie_mass_fraction(id=specie_id)

                    if nu < 0:
                        self.LiquidStream_dfdw_rxn_insta[:, rxn_insta_i, specie_i] = self.LiquidStream_Kw[:,rxn_insta_i] * np.abs(nu) * w ** (np.abs(nu) - 1)
                        for specie_jd in self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys():
                            nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_jd]
                            if specie_id != specie_jd and nu < 0:
                                w = self.LiquidStream.get_specie_mass_fraction(id=specie_jd)
                                self.LiquidStream_dfdw_rxn_insta[:, rxn_insta_i, specie_i] = self.LiquidStream_dfdw_rxn_insta[:, rxn_insta_i, specie_i] * w ** np.abs(nu)
                    else:
                        self.LiquidStream_dfdw_rxn_insta[:, rxn_insta_i, specie_i] = - np.abs(nu) * w ** (np.abs(nu) - 1)
                        for specie_jd in self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"].keys():
                            nu = self.LiquidStream.rxn_insta[rxn_insta_id]["Stoch"][specie_jd]
                            if specie_id != specie_jd and nu > 0:
                                w = self.LiquidStream.get_specie_mass_fraction(id=specie_jd)
    ...
    def __LiquidStream_dfdw_rxn_reversible__(self):
        self.LiquidStream_dfdw_rxn_reversible = np.zeros(shape=(self._num_of_samples_, self._num_of_rxn_reversible_, self._num_of_species_), dtype=np.float64)
        for rxn_i, rxn_id in enumerate(self.LiquidStream.rxn_reversible.keys()):
            for specie_id in self.LiquidStream.rxn_reversible[rxn_id]["Dependencies"]:
                specie_i = self.LiquidStream.specie[specie_id]["Index"]
                w_probe = self.LiquidStream.get_specie_mass_fraction(id=specie_id)
                dw_probe = np.maximum(0.01 * w_probe, 10 ** (-14))
                self.LiquidStream.set_specie_mass_fraction(id=specie_id, value=w_probe + dw_probe)
                forward_rate_pertubation, backward_rate_pertubation = self.LiquidStream.rxn_reversible[rxn_id]["Rate [kmol/m3.s]"](self.LiquidStream)
                rate_pertubation = forward_rate_pertubation - backward_rate_pertubation
                self.LiquidStream.set_specie_mass_fraction(id=specie_id, value=w_probe)
                self.LiquidStream_dfdw_rxn_reversible[:, rxn_i, specie_i] = (rate_pertubation - self.LiquidStream_rate[:, rxn_i]) / dw_probe

        RI = self._matrix_["R+"][self._num_of_rxn_insta_::, :]
        self.LiquidStream_dfdw_rxn_reversible = self.LiquidStream_dfdw_rxn_reversible - np.einsum("s,rw->srw", self.phi, RI)

    def __LiquidStream_dfdw_mass_balance__(self):
        self.LiquidStream_dfdw_mass_balance = self._matrix_["A broadcast"]
    ...
    def __dwdt__(self, w, t):

        self.LiquidStream.__mass_fractions_vec2dic__(w=np.maximum(w[None,:], 0))
        rho = self.LiquidStream.get_solution_density_kg_m3()[0]
        A_insta = self._matrix_insta_["A"]
        R_rev = self._matrix_rev_["R"]
        r = np.zeros(shape=(1, self._num_of_rxn_reversible_), dtype=np.float64)
        for i, id in enumerate(self.LiquidStream.rxn_reversible.keys()):
            r_forward, r_backward = self.LiquidStream.rxn_reversible[id]["Rate [kmol/m3.s]"](self.LiquidStream)
            r[:,i] = r_forward - r_backward

        Rr = np.einsum("wr,r->w",R_rev, r[0,:])
        ARr = np.einsum("cw,w->c", A_insta, Rr)
        self.__LiquidStream_Kw__()
        self.__LiquidStream_dfdw_rxn_insta__()
        dfdw = np.concatenate((self.LiquidStream_dfdw_rxn_insta, A_insta[None,:,:]), axis=1)[0,:,:]
        Hessian = np.linalg.pinv(dfdw)[:, self._num_of_rxn_insta_::]
        HARr = np.einsum("wc,c->w", Hessian, ARr)
        dwdt = HARr / rho
        return dwdt


class OLD_LiquidStream_HeatExchanger_PlugFlow_CounterCurrent(__Serializer__):
